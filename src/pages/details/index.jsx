...
export default function Details() {
    ...
    const { id } = useParams();
    const { recipeDetailsData, setRecipeDetailsData, favoutiteList, handleAddToFavourite }
    ...
    return (
        <div className="container mx-auto py-10 grid grid-col-1 lg:grid-cols-2 gap-10 ">
            ...
            <div className="flex flex-col gap-3">
                ...
                >{recipeDetailsData?.recipe?.publisher}</span>
                <h3 className="font-bold text-2xl truncate text-black">{recipeDetailsData?.recipe?.title}</h3>
                <div>
                    <button onClick={() => handleAddToFavourite(recipeDetailsData?.recipe)}
                       ...
                       {
                        favoutiteList && favoutiteList.length > 0 && favoutiteList.findIndex((item) => item.id === recipeDetailsData?.recipe?.id) !== -1 ?
